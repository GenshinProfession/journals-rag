...
// Standalone preview-react: direct to binding_case_api.py on 8899.
// When merged into frontend-writer, change to '/api/template-binding' (vite proxy).
...
export async function loadDocx(caseId: string, version: number): Promise<Blob> {
  const response = await fetch(`${BASE}/case/${caseId}/file?v=${version || Date.now()}`)
...
export async function insertImage(caseId: string, file: File, options: { anchorNodeId?: string; widthPx?: number; mode?: string } = {}): Promise<MutationResponse> {
  ...
  const response = await fetch(
    `${BASE}/case/${caseId}/image?anchor=${anchorNodeId}&width=${widthPx}&mode=${mode}`,
    {
      method: 'POST',
...
export async function replaceImage(caseId: string, nodeId: string, file: File, options: { widthPx?: number } = {}): Promise<MutationResponse> {
  ...
  const response = await fetch(
    `${BASE}/case/${caseId}/image/${nodeId}?width=${widthPx}`,
    {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/octet-stream', 'X-File-Name': file.name },
      body: file,
